...
if TYPE_CHECKING:
    from backend.db.models import SignalModel, PositionModel
...
class TelegramNotifier:
    ...
    def __init__(self, token: str, chat_id: str):
        if not TELEGRAM_AVAILABLE:
            logger.warning("python-telegram-bot не установлен. Используйте: pip install python-telegram-bot")
            self.bot = None
        else:
...
"""
    ...
    async def send_tp_hit(self, position: "PositionModel", tp_level: str, price: float) -> bool:
        ...
        pnl = position.unrealized_pnl_percent or 0
